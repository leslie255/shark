...
pub struct TokenStream {
    ...
    path: &'static str,
    source: SourceString,
    iter: Peekable<SourceCharIndices>,
...
impl TokenStream {
    ...
    fn parse_identifier(&mut self, start_index: SourceIndex) -> Option<Traced<Token>> {
    ...
    fn parse_number(&mut self, start_index: SourceIndex, first_ch: char) -> Option<Traced<Token>> {
        let mut end_index = start_index;
    ...
    fn parse_string_escape(
        ...
        match escape_char {
            ...
            'u' => {
                let (mut i, ch) = self.iter.next().ok_or(eof_err!().wrap((self.path, i)))?;
                if ch != '{' {
                    return Err(ErrorContent::UnicodeEscNoOpeningBrace.wrap((self.path, i)));
                }
                let mut val = 0u32;
                let mut overflowed: bool;
                while let Some((j, ch)) = self.iter.next_if(|&(_, c)| c != '}') {
                    i = j;
                    (val, overflowed) = val.overflowing_mul(16);
                    if overflowed {
                        return Err(ErrorContent::UnicodeEscOverflow.wrap((self.path, j)));
                    }
                    let digit = hex_char_to_digit!(ch, u32);
                    if digit > 15 {
                        return Err(ErrorContent::UnicodeEscNonHexDigit.wrap((self.path, j)));
                    }
                    (val, overflowed) = val.overflowing_add(16);
                    if overflowed {
                        return Err(ErrorContent::UnicodeEscOverflow.wrap((self.path, j)));
                    }
                }
                match self.iter.next() {
                    ...
                    None => Err(eof_err!().wrap((self.path, i))),
                }
            }
            'n' => Ok('\n'),
            'r' => Ok('\r'),
    ...
    fn parse_char(&mut self, start_index: SourceIndex) -> Option<Traced<Token>> {
        match self
            .iter
            .next()
    ...
    fn parse_string(&mut self, start_index: SourceIndex) -> Option<Traced<Token>> {
